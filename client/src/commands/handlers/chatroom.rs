...
use crate::{commands::{Command, CommandContext, CommandResult}, ui::CommandItem};
...
impl ChatroomCommand {
    ...
    async fn chatroom_loop(&self) -> Result<()> {
        ...
        input_handler.set_commands(vec![
            ...
            CommandItem { name: ":history", desc: "查看历史消息" },
            CommandItem { name: ":users", desc: "查看在线用户" },
            CommandItem { name: ":topic", desc: "查看或设置话题" },
            CommandItem { name: ":revoke", desc: "撤回消息" },
            CommandItem { name: ":bg", desc: "发送弹幕" },
            CommandItem { name: ":mutes", desc: "查看禁言列表" },
            CommandItem { name: ":raw", desc: "查看消息原文" },
            CommandItem { name: ":cost", desc: "查看弹幕价格" },
            CommandItem { name: ":disconnect", desc: "断开连接" },
        ...
        loop {
            ...
            match input_handler.start_input_loop(&format!("{}", "聊天室> ".green().bold())).await? {                
                Some(input) => {
                    if input.is_empty() {
                        continue;
                    }
    ...
    async fn register_message_handler(&self) -> Result<()> {
        ...
        let result = self.context.client.chatroom.add_listener(move |data| {
            ...
            tokio::spawn(async move {
                match data.data {
                    ...
                    ChatRoomDataContent::Barrager(barrager) => {
                        ...
                        let (r, g, b) = if let Some(stripped) = color_str.strip_prefix("rgba(").and_then(|s| s.strip_suffix(')')) {
    ...
    async fn send_message(&self, message: &str) {
        ...
        let result = self.context.client.chatroom.send(Cow::from(message), None).await;
        if !result.success {
            println!("{}: {}", "发送失败".red(), 
                     result.message.unwrap_or("未知错误".to_string()));
        }
    ...
    async fn send_barrage(&self, content: &str, color: &str) {
        ...
        let result = self.context.client.chatroom.send_barrage(content, color).await;
